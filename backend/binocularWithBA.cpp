...
void main()
{
	try {
		...
		cout << "Extracting features with SIFT finish." << endl;
		//对所有图像进行特征匹配
		match_features(images_record, matches_record, descriptor_for_all, matches_for_all);
		cout << "Matching image features finish." << endl;
		...
		save_structure(folderpath + "\\structure.json", rotations, motions, structure, colors);

		cout << "Build Success" << endl;

	}
	catch (Exception e) {
		cout << "Build failed. Please select other images." << endl;
	}
